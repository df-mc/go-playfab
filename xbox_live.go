...
type XBLIdentityProvider struct {
	// TokenSource is used for obtaining a [xsapi.Token] that relies on the RelyingParty.
	TokenSource xsapi.TokenSource

	// Config is the base LoginConfig to be used for signing in to the PlayFab using the TokenSource.
	Config LoginConfig
}

// Identity signs in to PlayFab using a [xsapi.Token] obtained from the TokenSource that relies on the
...
func (prov XBLIdentityProvider) Identity() (*Identity, error) {
	...
	return prov.Config.Login("/Client/LoginWithXbox", loginConfig{
		LoginConfig: prov.Config,
